...
-(void)writeDataToDevice:(NSString *)serviceUuid characteristicUuid:(NSString *)characteristicUuid isNoResponse:(BOOL) isNoResponse data:(NSData *) data{
    ...
    //如果设置了CBCharacteristicWriteWithoutResponse，则写入数据之后不会进入写入回调
    // 须判断数据长度， 超过 20，则须分段发送/写入
    NSUInteger SendDataOnce_MaxLen = 20;
    if([data length] > SendDataOnce_MaxLen){
        NSUInteger times = [data length] / SendDataOnce_MaxLen;
        NSUInteger cycleIdx = (0 == times) ? times : (times + 1);  // 分段发送次数
        NSRange subRange;
        NSInteger lastSubDataLen; // 最后一段数据包
        for (NSUInteger i = 0; i < cycleIdx; i++) {
            if(0 == times) {
                subRange = NSMakeRange(i * SendDataOnce_MaxLen, SendDataOnce_MaxLen);
            } else {
                if( i == (cycleIdx - 1) ) {
                    lastSubDataLen = [data length] - i*SendDataOnce_MaxLen;
                    subRange = NSMakeRange(i * SendDataOnce_MaxLen, lastSubDataLen);
                } else {
                    subRange = NSMakeRange(i * SendDataOnce_MaxLen, SendDataOnce_MaxLen);
                }
            }
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.05f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                // 往硬件蓝牙设备【写数据】调用方法
                [self.peripheral writeValue:[data subdataWithRange:subRange] forCharacteristic:characteristic type:isNoResponse? CBCharacteristicWriteWithoutResponse:CBCharacteristicWriteWithResponse];
            });
        }
    }else{
