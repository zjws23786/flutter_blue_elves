...
public class Device {
    ...
    private int mCount = DEFAULT_WRITE_DATA_SPLIT_COUNT;
    ...
    public void writeDataToDevice(String serviceUuid,String characteristicUuid,boolean isNoResponse,byte[] data){
        if (data.length > 20){
            ...
            write(serviceUuid,characteristicUuid,isNoResponse);
        }else{
            BluetoothGattCharacteristic mBleGattCharacteristic = getBluetoothGattCharacteristic(serviceUuid,characteristicUuid);
            if(mBleGattCharacteristic!=null){
    ...
    //写入
    private void write(String serviceUuid,String characteristicUuid,boolean isNoResponse) {
...
//                    System.out.println("hjz 写入成功");
                    ...
                    if (mDataQueue == null || mDataQueue.peek() == null) {
                        return;
                    }else{
                        byte[] data = mDataQueue.poll();
                        if(characteristic!=null){
                            characteristic.setValue(data);
                            characteristic.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT);//是否是有响应写
                            mBleGatt.writeCharacteristic(characteristic); //发送
                        }
                    }
                }
                 else //如果失败了
                {
//                    System.out.println("hjz 写入失败");
